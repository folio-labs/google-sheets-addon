...
  var cell = spreadsheet.getRange(1,1,spreadsheet.getLastRow(),spreadsheet.getLastColumn()).getValues();
  var grantedToArray = new Array(spreadsheet.getLastRow()-2);
  var xx = 0;
  dataAll.forEach(function( row, index ) { 
    
    grantedToArray[index] = new Array(lastColumn - 3);
    var yy = 0;
    for (var i = 4; i <= lastColumn; i++) {
      var dex = 1;
      row.grantedTo.forEach(function(row) {
        if (row == cell[0][i-1]) {
          grantedToArray[xx][yy] = "X";
          dex++;
        }
        else {
...
  spreadsheet.getRange(3, 4, spreadsheet.getLastRow()-2, spreadsheet.getLastColumn() - 3).setValues(grantedToArray);
  
  
...
      ui.alert("Error calling perms/users. Response: " + permResponse.getContentText()); 
      return;
    }
